...
## E040 — 세션 transcript 파싱 시 초기 요약본을 최종본으로 오인 (2026-04-20)

**증상**:
- POMADE 플리 #1 9곡 풀 가사 Notion 저장 작업 중, 마스터 오가 풀 가사를 직접 전달했는데도 초기 요약 버전(Verse 1 + Chorus까지만)을 저장
- 2회 지적 받고 복원 ("풀가사인데?" → 일부 복원 → "1번 3번이나 전달줬는데?" → 완전 복원)

**원인**:
- 이전 세션 transcript(`2026-04-20-14-59-20-pomade-reverse-eng-playlist1.txt`)에 같은 곡의 데이터가 여리 타임스탬프로 존재:
  - Line 82, 4967: 초기 요약 (Intro + V1 + Chorus)
  - Line 14:16, 5036, 5520: 마스터 오가 전달한 풀 버전 (Outro까지)
- Claude가 초기 타임스탬프를 최종본으로 오인

**교훈**:
1. **세션 transcript에서 데이터 탐색 시 가�$5 나중 타임스탬프의 원본을 우선 확인**
2. **마스터 오가 "데이터 전달 줬다"고 하면 즉시 transcript 최신→과거 탐색**
3. **풀 가사는 하상 [Outro] 또는 마지막 섹션까지 있는지 확인 후 저장**
4. **의심스러울 때 마스터 오에게 확인 요정 (추측으로 저장 금지)**

**재발 방지**:
- 풀 가사/풀 스타일 저장 시 마지막 섹션(Outro, Final Chorus 등) 명시적 존재 확인
- "..." 또는 "반복"으로 끝나는 섹션은 불완전 저장 플레그

**해결**:
- P1_01 NO MERCY: Bridge 6줄 + Final Chorus + Outro 복원
- P1_02~P1_09: 전수 풀 가사 확인 후 저장
